...
The usage scope is also provided when they are not used throughout the paper.
\begin{table}[h!]
\caption{Notations}
\label{tab1}
\centering
...
For the second term in the bracket, we propose $p(C_i)$ which is a size-related weight for cluster $C_i, i\in{1,\cdots, M}$.
There is, 
$$
p(C_i) = \rho(||C_i|-\delta|)
$$
where $\rho$ is a positive increasing function with respect to the the difference between $C_i$'s size and the desired size $\delta$.
...
%\end{array} \right.
%$$
%where $\rho$ is an increasing function and there is $ 0 < \rho(1)< \rho(2) < \cdots $.
...
\eIf{$\exists j\in \text{\upshape Nb}(i)\setminus \Lambda$, \text{\upshape such that} $d_i > d_j$}{
	return 0;}{
	\If{$\nexists j\in \text{\upshape Nb}(i)\setminus \Lambda$, \text{\upshape such that} $d_j$ \text{\upshape equals}  $ d_i$}{
	$\tau_1 \leftarrow j$
	}
}
...
\eIf{$\exists j\in \tau_1$, \text{\upshape such that} $g_i < g_j$}{
	...
	{\If{$\nexists j\in \tau_1$, \text{\upshape such that} $g_j$ \text{\upshape equals} $g_i$}{
		$\tau_2\leftarrow j$
		}
	}
...
\KwOut{one cluster $C\in S_i$, afterwards node $i$ notifies all its claiming clusters in $S_i$ about its affiliation decision.
}
\While{$i$ \text{\upshape has not chosen a cluster, or} $i$ \text{\upshape has chosen cluster} $\tilde{C}$, \text{\upshape but} $\exists C'\in S_i (C'\neq \tilde{C})$, \text{\upshape which has} $|K(C'\setminus i)|-|K(C')|<|K(\tilde{C}\setminus i)|-|K(\tilde{C})|$}{
	\eIf{$\exists$ \text{\upshape only one} $C\in S_i$, $C = \argmax \sum\limits_{C'\in \mathcal{S}_i\backslash C} (|K(C'\setminus i)| - |K(C')|)$}{
			return $C$;
		}{
...
%Given any clusters formation with respect to the instance of the maximum weighted k-set packing problem, after removing the singleton clusters, we can easily find the direct association between the remaining clusters and the sets in the instance $\mathcal{I}\subseteq \mathcal{Q}$. 
%The clusters in the CRN correspond to the sets in $\mathcal{I}$ according to the mapping between the node IDs in the clusters and the elements in the sets.
...
%Now suppose there is a set of pairwise disjoint clusters which constitute the CRN $\mathcal{N}$, and the sum of $f(C)$ is greater than $\lambda$, the robust clustering black box will say yes, then the instance $\mathcal{I}$ is the answer to the maximum weighted k-set packing problem.
