...
The range can also be expressed as $[\delta-u, \delta+u]$, where $\delta$ is the desired cluster size, $u$ is a constant and $\delta-u > 0$.
...
From Figure~\ref{control_msg} we can see the upper bound on the number of control messages, which are involved in the variants of ROSS, is still smaller than that involved in SOC.
...
As shown in the Table~\ref{Simulation_para}, the desired size equals to 60\% of the average number of neighbors.
The transmission range of CR to $A/5$ while the primary user transmission range is set to $2A/5$.
