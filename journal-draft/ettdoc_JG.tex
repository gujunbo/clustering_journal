...
In CRNs, formed clusters maintain a set of unlicensed channels which are validated by every CR node in that cluster, meaning that the channel is perceived as not been occupied by a primary user.
...
Throughout this paper, we call the newly proposed distributed schemes \textit{variants of ROSS}.
%
%
...
Summarizing, this work and SOC deem cluster robustness just to be the number of CCs per cluster, and don't consider the sustainability of clusters against increasing activity of primary users, leaving the issue of cluster robustness open.

% XXX Not happy with this last sentence. In my view, robustness is provided by the number of CCs and therefore both works address robustness. However, they do not address the issue of the size of the cluster, where we have been saying further up in the introduction that a small cluster looses sensing accuracy, so what these two works do not address is the balance between sensing accuracy and robustness (which we have been discussing much in the introduction). Is that maybe what is meant here?

% XXXX 
%Their biggest problem is, that they stick to the 'the number of CCs' metric and claim they achieve the robustness by obtaining the 'large number of CCs'.
%It is fair to say robustness is provided by the number of CCs, but only the amount of CCs doesn't decide how robust the clusters are. It is possible that the cluster with more CCs can be destroied earlier than the other clusters under the intensity of PRs' activity. This situation is illustrated in Fig 8 and 10.
%Except for the amount of CCs, robustness is also decided by cluster sizes and the compositiomn of the clusters.
%In the simulation section, one of the conclusion is the number of CCs is not an accurate metric to measure robustness (consider Fig 8 and 10). Nevertheless, in our schemes, we still consider to increase 'the number of CCs' to improve robustness. 
% XXXX 
...
We also summarize all used notation in Table~\ref{tab1}.
\begin{table}[h!]
\caption{Notations}
\label{tab1}
\centering
...
Robustness of the clusters is the ability to uphold with the influence of the active primary users, \ie how many secondary users are not included in any cluster.
To achieve better robustness, we use the amount of CCs as metric and try to increase it. 
Meanwhile, the sizes of the formed clusters should be regulated, so that the clusters can not only benefited from the spectrum sensing accuracy from cooperative sensing, but achieve a large set of CCs.
...
\item the union of all nodes in all clusters in $\mathcal{T}$ is $\mathcal{N}$
...
When the cluster size is larger or smaller than the range $\big[\delta_1, \delta_2\big]$, $f(C)$ is defined as 0.
...
We obtain the set of $\mathcal{S}$ which contains all the clusters.
These clusters satisfy the definition of cluster in Section~\ref{sec:model}, and they basically form a powerset of $\mathcal{N}$, which nevertheless are restricted by connectivity and spectrum availability.
...
$y_i$ and $x_{ij}$ are binary variables.
