...
%XXXXXX%
%Di: The first paragraph has been revised based on the first batch of comments for the camera-ready version, but somehow get lost in the last version. I take some parts from your revision and keep some from my revision.
...
$q_{ij}$ is a constant and it equals $|K(C_i)|$ when node $j\in C_i$.
$q_{ij}$ equals zero when node $j\notin C_i$.
...
As to the objective function, the sum of the first term in the bracket over all clusters in $\mathcal{S}$ and all CR node in $\mathcal{N}$ is the sum of CCs of the clusters which constitute the CRN.
