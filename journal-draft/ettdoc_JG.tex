...
Note that here we adopt the desired cluster size in stead of the upper and lower bounds for cluster sizes which is described in Definition~\ref{problem}.
%Because in implementation, we usually need a large range to guarantee a feasible result for the optimization problem.
With $\delta$ and the deviation from it, we actually set a large range of cluster sizes, which guarantees the feasibility of the problem as we allow the singleton cluster to appear, meanwhile a better control of the resulted sizes by setting weights for the formed clusters based on their deviations from the desired size.
