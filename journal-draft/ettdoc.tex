...
In this paper, the robustness of a cluster is the ability to sustain with the existence of primary users.
%
...
Here we give a brief review on the clustering in CRN, with emphasis on the theme of robust clustering.
%%[crn for clustering] 
...
In Figure~\ref{fig1}, some primary users are randomly deployed in the network, and they are free to choose one channel out of the set of licensed channels $\mathcal{K}$ to operate.
...
Given a CRN, it takes at most $N$ steps before every secondary user either becomes cluster head, or gets included into at least one cluster.
\end{theorem}
Here, by \textit{step} we mean Algorithm~\ref{clustering:theorem} is executed by a secondary user for one time.
Note that the minimum number of steps is 1, \eg when a cluster head is elected and all the other nodes are within its neighborhood.
The Proof on the upper bound of the steps is given in Appendix~\ref{proof_clustering:theorem}.
%The proof also illustrates the maximal time needed to conduct Algorithm~\ref{clustering:theorem}. 
Consider an extreme scenario, where all the secondary nodes sequentially execute Algorithm~\ref{alg0}, \ie they constitute a chain as discussed in the example in the proof.
%
Assuming there is a token in the CRN, one node is allowed to execute Algorithm~\ref{alg0} only when it holds the token, and it then transfers the token to the neighboring node on the chain.
...
The constant parameter $t$ is usually larger than 1 and is dependent on the network density and CR nodes' transmission range, in particular, $t$ is between 1 and the ratio between the average number of neighbors and the desired size.
...
Then certain formed clusters may be affected as their CCCs which are obtained due to false negative will be revoked.
...
In this part of simulation, there are 10 primary users and 20 CR users dropped randomly (with uniform distribution) within a square area of size $A^{2}$, where $A$ is a positive value and we set the transmission ranges of primary and CR users to $A/3$.
%C1-10, C1-9
...
Figure~\ref{false_negative_ccc} shows the average number of CCCs decrease slightly when the false negative rate increases.
...
\begin{figure}[!h]
  ...
  \caption{The number of CCCs per non-singleton cluster with the presence of spectrum sensing false negative}
  %, there are $x=6$ when $N=100$, $x=12$ when $N=200$, $x=21$ when $N=300$, which is around $2/3$ of the number of average neighbours.
  \label{false_negative_ccc}
\end{figure}
%The influence of the false negative on the sustainability of the clusters can be seen from Figure~\ref{false_negative_CDF}.
For all the schemes, when false negative increases, the number of singleton clusters increases accordingly.
%As to the ROSS variants, the changes of the CDF curves are resulted from the increasing singleton clusters.
...
First, different from the assumption adopted in all the previous works, the average number of CCCs per cluster can tell the robustness against the increasing influence of the primary users.
