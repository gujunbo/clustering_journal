...
\item the sum of the $f(C)$ is the maximal, where $C\in \mathcal{T}$ and meanwhile the cluster sizes fall in the scope $\langle\delta_1, \delta_2\rangle$.
...
We will show that any instance $\mathcal{I}$ of a weighted $k$-set packing problem, \ie a collection of sets, can be transformed into an instance of a robust clustering for a CRN within polynomial time.
W.l.o.g. let set $\mathcal{G} = \{ 1, \ldots , N \}$, and redeem there are $N$ CR nodes in the CRN which is to form clusters. 
