...
Figure~\ref{false_negative} shows the average number of CCCs are decreased slightly.
The reason is that in the process of 
A channel on a CR, which is due to false negative, is very likely ruled out during the interaction in the neighborhood.
Figure~\ref{false_negative_ccc} is shows the size distribution of the ROSS-DGA, ROSS-$\delta$-DGA and SOC with the existence of spectrum sensing false negative.
\begin{figure}[!h]
  \centering
...
When false negative exists and increases, the number of singleton clusters increases accordingly.
Actually as to the ROSS variants, the changes of the CDF curves are due to the increasing singleton clusters.
In contrast to the ROSS variants, the clusters resulted from SOC are affected by the sensing errors greatly, \ie when false negative rate is 30\%, a lot more small clusters are generated than that when other false negative rates present.
ROSS variants are resilient to the false negative through the negotiation within the neighborhoods, whereas SOC demonstrates obvious divergence when certain rate of false negative exists.
