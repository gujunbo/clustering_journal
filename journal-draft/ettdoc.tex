...
In this paper robust clustering in CRN is analyzed and both centralized and distributed schemes are proposed.
This paper is based on our previous work ROSS (Robust Spectrum Sharing)~\cite{Li11_ROSS}, but fundamental extensions are made in several aspects.
This paper provides a comprehensive analysis of the robust clustering problem, analyze and investigate the influence of cluster size and the number of CCs on cluster robustness.
We claim that the cluster robustness against the primary users is no longer indicated merely by the average number of CCs of clusters, but by the ability of the clusters to uphold when the primary users' activity becomes more active.
The involment of cluster size enables us to solve the problem of clusters size devergence, which appears in~\cite{Li11_ROSS} and~\cite{LIU_TMC11_2}.
These new extensions are made on the basis of ROSS and its light weight version, the latter involves less overheads thus is more suitable for the scenario where fast deployment is desired.
...
\section{Related Works and the Relation with This Paper}
\label{related_work}
...
To achieve better robustness, the formed clusters should have more CCs.
...
ROSS consists of two cascaded phases: \textit{cluster formation} and \textit{membership clarification} as shown in Figure~\ref{flowChartROSS}.
\begin{figure}[ht!]
  \centering
...
Actually during the whole phase I, whenever a CR node is decided to be a cluster head thus accordingly forms a cluster, or its cluster's composition is changed, the cluster head will broadcast the new information about its cluster, which includes the sets of available channels on all its cluster members.
...
%In one special scenario, \eg when a cluster head is elected and all the other nodes are within its neighborhood, the number of steps is 1.
