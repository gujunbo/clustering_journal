...
Except for benefiting the dynamic spectrum sharing in CRN, clustering contributes to the other wireless networks in many aspects, which has already been discussed in many works. [CITATION MISSING]
...
The drawbacks of this scheme are as follows, although the adopted metric considers both cluster size and the number of CCs, cluster formation can be easily dominated by only one factor, e.g. a node which can access many channels may exclude its neighbor and form a cluster by itself. (THIS SENTENCE IS TOO COMPLICATED. BREAK IT DOWN TO 2 OR 3 PLEASE.)
...
First, this paper renews the definition of robustness of clusters in a CRN, which is no longer the average number of CCs of clusters, but the ability of the clusters to sustain when the primary users' activity becomes more dynamic. (BAD FORMULATION TO SAY "renew". Changing definitions itself does not impose any benefit. Is there any an improval?)
...
\item the sum of the $f(C)$ is the maximal, where $C\in \mathcal{T}$ and meanwhile the cluster sizes fall in the scope $\big[\delta_1, \delta_2\big]$.
...
The decision version of this problem is to determine whether there is a set of clusters, say $\mathcal{X}$, so that $\sum_{\forall C\in \mathcal{X}} f(C) \geqslant \lambda$ where $\lambda$ is a real number.
...
Given an instance of the weighted $k$-set packing problem, \ie a collection of sets of distinct integers $\mathcal{Q}=\{S_1,S_2,\cdots,S_m\}$, the integer weight $\omega(S)$ for set $S$ and a integer $\lambda$, we will construct an instance of a CRN robust clustering problem within polynomial time.
W.l.o.g. we let set $\cup_{\forall i\in\{1, \ldots, m\}}S_i = \{ 1, \ldots , N \} = \mathcal{N}$.

We will construct the CRN and the clusters as follows:
For every set $S$ whose size is larger than 1, a corresponding cluster of CR nodes is generated.
The CR nodes within this cluster can pair wise communicate and the IDs of these CR nodes correspond to the elements in $S$, besides, we allocate the $|\omega(S)|$ channels to all the CR nodes in this cluster, and the channels are on the spectrum band which is exclusive for this cluster.
For every set $S$ which contains only one element, \ie $S =\{i\}$, a cluster composed with two CR nodes are created.
One CR node's ID is $i$, the other CR node is the dummy node of the former and its ID is $N+i$.
$|\omega(S)|$ channels from the exclusive spectrum band for this cluster are allocated to these two CR nodes.
In the end, we construct the singleton clusters, where the IDs of the CR nodes correspond to the elements in $\mathcal{N}\backslash \cup_{S_i}$.
...
After removing the singleton clusters, we can easily find the natural association between the clusters in the CRN and the sets in $\mathcal{Q}$. 
The clusters which contain dummy CR nodes correspond to the sets which contain only one element, and the other clusters correspond to the sets in $\mathcal{Q}$, whose elements are the node IDs of the corresponding clusters.
Then the sum of the weights of such sets equals to the sum of $f(C)$ and thus greater than $\lambda$.
