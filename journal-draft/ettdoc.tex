...
Consequently, a cluster which secures more CCs will be more robust to random primary user activity and anticipate a longer life expectancy. 
...
However, this work does not consider robustness improvement of clusters, while also ignoring messaging overhead.
...
%
In~\cite{Li11_ROSS} we propose a distributed clustering scheme ROSS (Robust Spectrum Sharing) under the game theoretic framework. 
...
Meanwhile, the sizes of the formed clusters should be regulated, \ie they don't diverge from a given value greatly.
...
As to the node which is excluded form a cluster due to guaranteeing CC or size control, its \textit{individual connectivity degree} is restored, but it will not include its former cluster head, even if it becomes cluster head as regulated in~\ref{alg0}.
\end{proof}
...
\end{theorem}
Phase I ends when no more secondary users are becoming cluster head.
...
If we use the number of secondary users residing in a cluster as the numerical representation of cluster robustness, then the ROSS based schemes are 5\%, 30\% and 230\% more robust than SOC, where the numbers of newly added PR are 10, 40 and 80 respectively.
...
For all the schemes, when false negatives increase, the number of singleton clusters increases accordingly.
%As to the ROSS variants, the changes of the CDF curves are resulted from the increasing singleton clusters.
The clusters from SOC are affected by the sensing errors greatly.
