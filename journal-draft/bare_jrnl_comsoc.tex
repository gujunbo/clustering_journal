...
The CR users opportunistically a set of licensed channels, which is denoted as $\mathcal{K}$. 
...
If a CR node locates within the transmission range of primary user which is working, that CR node is not allowed to use the channel which is being used by that primary user.
...
As the transmission range of primary users is limited and secondary users are located at different locations, different secondary users may have different views of the spectrum availability, i.e., for any $i, j \in \mathcal{N}$, $K_i = K_{j}$ does not necessarily hold.	
%
...
The autonomous decisions of the debatable CR nodes' raise the concern on the endless chain effect.
For instance, 

debatable nodes may never stop changing their affiliations, because a debatable nodes' choice is based on the members of its claiming clusters, and the members are changed by other debatable nodes' choices.
