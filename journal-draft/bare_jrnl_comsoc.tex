...
When no cluster head appears, this series of nodes with monotonically decreasing $d_i$ might continue to grow, but finally ceases when either the individual connectivity degree is zero, or every secondary node has already been in the series of nodes.
...
As $\omega$ is the end node and does not have neighbouring nodes with lower individual connectivity degree $D$, $\omega$ becomes cluster head and incorporate all its one-hop neighbours, including the node before it in the series (here we assume that every new formed cluster has common channels).
After that, the node recruited into cluster will set its connection degree $D$ to $M$, which enables the node further down in the list to become a cluster head.
In this way, all the nodes in the series are in at least one cluster in the inverse sequence.
This result contradicts the initial assumption and proves the claim stated above.
Meanwhile, through this proof, we know that within at most $N$ steps, all the nodes belong to certain clusters.
\end{proof}
