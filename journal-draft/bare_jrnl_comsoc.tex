...
Till now, when there is no cluster head identified, the unclustered nodes, \ie $\alpha$, $\bata$ form a linked list, where their connectivity degrees monotonically decrease.
...
The nodes which join a cluster set their individual connection degrees to $M$, which enables the node immediately precede in the listto become a cluster head.
...
(This paragraph should be moved out of the proof.)
The procedure of this proof also illustrates the time needed to complete phase I. 
Consider an extreme scenario, where all the secondary nodes sequentially execute Algorithm~\ref{alg0}, and constitute one list as that in the above example.
If we see a secondary user \textit{becoming a cluster head}, or \textit{becoming a cluster member} as one step, then there are $N$ steps for this scenario to complete phase I.
If one step can be finished within certain time, we can give the maximum time needed to conduct the phase I of ROSS.
