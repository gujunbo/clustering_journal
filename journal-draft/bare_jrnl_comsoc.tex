...
A cluster $C$ is a set of secondary nodes, which consists of a cluster head $h_C$ and a number of cluster members.
...
$ K(C) = \cap_{i\in C} K_i$, $K(C)$, which denotes the set of CCs in cluster $C$.
...
As introduced in Section~\ref{intro}, in order to be robust against primary users' activity, the formed clusters should have more CCs to expect longer time expectancy.
