...
The robustness of the clusters is obtained from the multiple CCs.
...
But as all the other clustering schemes, this work doesn't intervene the outcome of the very small or very large clusters which are not desirable, although the cluster sizes are not as divergent as SOC.
...
$\delta_1\leq|C_i|\leq \delta_2, \bigcup_{C_i\in \mathcal{S}'} C_i = \mathcal{N}$ and $C_{i'}\cap C_i =\emptyset$ where $C_{i'}, C_i\in \mathcal{S}'$, $i'\neq i$.
...
In particular, considering $|\mathcal{S}|=M$, $C_i\in \mathcal{S}$ means the $i$ th cluster in $\mathcal{S}$, and $\delta$ is the desired size, the size related weight for each cluster is given as follows,
$$
...
Then, a central node (or controller) with the knowledge of all CR nodes (and also the possible clusters) will solve the problem based on the following formulation. 
...
\begin{equation}
\begin{aligned}
     &\min\limits_{w_i, x_{ij}} && \Sigma_{j=1}^N\Sigma_{i=1}^M (w_i\cdot t_{ij}) \\
...
Replacing $t_{ij}$ with $p_i(C_i) - \frac{q_{ij}}{|C_i|}$, the objective function becomes,
\begin{equation}
\begin{aligned}
     &\min\limits_{w_i, x_{ij}} && \Sigma_{j=1}^N\Sigma_{i=1}^M (-w_i\cdot \frac{q_{ij}}{|C_i|} + w_i\cdot p_i(C_i)) \\
\notag
\end{aligned}
\label{centralized_opt}
\end{equation}
