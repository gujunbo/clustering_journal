...
The availability of unlicensed channels which are available for every member in a cluster decides the robustness of that cluster against the licensed users' influence.
...
%
Our proposed distributed clustering scheme outperforms the comparison scheme in terms of cluster robustness against the primary users, convergence speed and volume of control messages.
...
\section{Clustering Algorithm: ROSS}
\label{ross}

In this section we introduce the distributed clustering scheme ROSS which leads to robust clusters against PU' influence.
...
Given a CRN, every secondary user is included into at least one cluster within $N$ steps. (COMMENT: $N$ has to be defined within the theorem!!)
\end{theorem}
Here, by \textit{step} we mean one application of Algorithm~\ref{clustering:theorem} by a secondary user.
...
It is possible that certain formed clusters don't own CCCs, then we solve this problem with the following method.
...
With the increase of CR users, network density increases linearly (the Y axis label is the average number of neighbours), and the number of formed clusters also increases and approaches to the the upper bound of 25 which complies with the estimation.
...
At this point of time, debatable node $j$ may rejoin cluster $C$ due to the changes in $S_j$, then both node $i$ and $j$ face the \textit{same}\footnote{Actually it is not totally same as before, as there are some new changes within $S_j$.} situation again.
...
The objective is a sum of two parts, the first part is the sum of products of cluster size and the corresponding number of CCCs, which is the sole metric adopted by the scheme SOC~\cite{Lazos09}.
...
\end{array} \right.
$$
The second constraint regulates when the $i$th legitimate cluster $C_i$ is chosen, the number of elements which are 1 in the $i$th row of the matrix $X$ is $|C_i|$.
%
...
The resulted clustering solutions from ROSS-DGA/DFA and SOC are shown in Fig.~\ref{fig4} and Fig.~\ref{fig:final_clustering_soc} respectively.%, clusters are $\{A,B,C,D,G\},\{E,F\},\{H\}$, and the numbers of CCCs are 2, 3, 4.
...
Hence, we only compare our proposed schemes with SOC  to show the ROSS's merits as a distributed scheme, and compare with the centralized scheme to examine the gap with the global optima.
...
We will present the distribution of CRs residing in the formed clusters with different sizes.
...
Figure~\ref{ccc_per_nonsingleton} shows the average number of the CCCs of non-singleton clusters,
...
from which we can see the centralized schemes outperform the distributed schemes.
...
As to the variants of ROSS, greedy mechanism increases CCCs in non-singleton clusters significantly.
...
\begin{itemize}
\item The centralized scheme with desired size of two generates the most robust clusters, and SOC results in the most vulnerable clusters.
...
\item Greedy algorithm improves the survival rate. 
...
We omit the control messaged involved in the process of neighbourhood discovery, which is the premise for any clustering scheme.
...
As to the process of dissemination, in an extreme situation where all the gateway and the backbone nodes broadcast, the number of transmission will be $h + m$, where $h$ is the number of cluster heads, $m$ is number of debatable nodes, $d$ is the average number of demanding clusters for each debatable node.
...
The desired sizes adopted are listed in the Table~\ref{Simulation_para}.
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=.8\linewidth]{ccc_large_scale_color.pdf}
...
When $N=100$, variants of ROSS have 30\% less CCCs than SOC, but this gap is decreased significantly when $N$ is 200 and 300, \ie when $N=300$, the number of CCCs achieved by ROSS variants (except for ROSS-$\delta$-DFA) is almost the same as that resulted from SOC.
...
Figure~\ref{singleton_clusters_100} illustrates the increasing trend of singleton clusters with the increase of PUs, when $N=100$.
...
From the Figure~\ref{singleton_clusters_100} and \ref{singleton_clusters_300}, we can conclude that the clusters obtained from the variants of ROSS are more robust than SOC.
...
\subsubsection{Cluster Size Control}
As Fig.~\ref{nClusters_largeNetwork} shows, when the network scales up, the number of formed clusters by ROSS increases by smaller margin.
This result coincides with the analysis in Section~\ref{cluster_pruning}, that the number of formed clusters saturates when the network scales.
...
We consider a CRN which can be represented as a connected graph. (COMMENT: Why is it necessarily connected? No special case where the graph has two components?)
To simplify the discussion, we assume the secondary users have unique individual connectivity degrees. Each user has an identical ID and a social connectivity degree.
This assumption is fair as the social connectivity degrees and node ID are used to break ties in Algorithm~\ref{alg0}, when the individual connectivity degrees are unique, it is not necessary to use the former two metrics. (COMMENT: What do you mean by breaking ties?? I don't see why your argument holds.)
...
Then there is at least one node $\beta\in \text{Nb}_\alpha$ such that $d_{\alpha} > d_{\beta}$. Now, we distinguish between two cases: 
%Otherwise, node $\alpha$ is eligible to form a cluster. COMMENT: unnecessary sentence!
If $\beta$ becomes cluster head, node $\alpha$ is included.
If $\beta$ is not a cluster head, i.e., $\beta$ is not in any cluster (COMMENT: $\beta$ not being a cluster head and $\beta$ not belonging to any cluster is not the same statement!!!), we can repeat the previous analysis made on node $\alpha$, and deduce that node $\beta$ has at least one neighbouring node $\gamma$ with $d_{\gamma} < d_{\beta}$.
When no cluster head appears, this series (COMMENT: what is that? sequence?) of nodes with monotonically decreasing connectivity degrees might continue to grow. Finally, it ceases (COMMENT: not a good/suitable word!!) when either the individual connectivity degree is zero, or every secondary node has already been in the series (COMMENT: sequence?) of nodes.
...
Now, we can see that the node $\omega$ is the last element of this sequence.
As $\omega$ does not have neighboring nodes with lower individual connectivity degree $D$, $\omega$ becomes the cluster head and incorporate all its one-hop neighbours, including the node before (COMMENT: you mean $\alpha$?) it in the series (here we assume that every newly formed cluster has common channels).
After that, the node that joined a cluster will set its connection degree $D$ to $M$, which enables the node further down in the list (WHAT ??? What is further down and which list??? I cannot follow.) to become a cluster head.
In this way, all the nodes in the series are in at least one cluster in the inverse sequence (? inverse of?), which contradicts the assumption.
Meanwhile, through this proof, we know that within at most $N$ steps, all the nodes belong to certain clusters. (COMMENT: No, we don't! Where was that shown?)
\end{proof}
...
%\end{itemize}
We show the existence of a polynomial-time algorithm $\sigma$ that transforms any instance $\mathcal{S}$ (COMMENT: be careful - there is an overloading of $\mathcal{S}$ of a weighted $k$-set packing into an instance $\sigma(\mathcal{S})$) of centralized clustering such that ... (to be completed - Erwin can do that) 
...
\begin{itemize}
\item Given a collection $\mathcal{Q}$, on basis of which we construct a CRN. (I DON'T UNDERSTAND THAT. What do you mean?)
...
We regard each set in $\mathcal{Q}$ as a cluster, whose number of CCCs is equal to the weight of that set.
...
Formally, for each set $s_i\in \mathcal{S}$, we create $s_i' = s_i \times \{ 1,2\}$. %for instance, given $s_i=\{1, 4, 6\}$, the dummy set $s_i'$ is $\{1,1',4,4',6,6' \}$.
...
We locate these CR nodes besides the CR nodes with the same IDs (THAT DOESN'T MAKE SENSE. THEN THE ID IS NOT UNIQUE ANYMORE.) in the CRN built in step 1, and there is connection between the CR node and its dummy node (the one CR node and its dummy node can be seen as two transceivers at one node). (YOU DON'T NEED TO GIVE PRACTICAL MOTIVATION IN A HARDNESS-PROOF.)
