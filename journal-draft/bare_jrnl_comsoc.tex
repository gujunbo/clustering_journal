...
We consider wireless sensor nodes distributed on a two-dimensional Euclidean plane. We denote the set of these nodes as $V$ which is partitioned into sets of primary users $V_P$ and secondary users $V_N$, i.e., $V~=~V_P~\dot\cup~V_N$. These users share a number of non-overlapping licenced channels according to the spectrum overlay model. The set of these licenced channels is denoted as $\mathcal{C}$. Secondary users are allowed to transmit on a channel $c \in \mathcal{C}$ only if every primary user $p \in V_P$ that has access to channel $c$ is idle. Further, we consider a \textit{cognitive radio ad-hoc network} which consists of all secondary users and does not contain any primary user.
...
Secondary users conduct spectrum sensing independently and sequentially on all licenced channels. In our system model, we assume that a secondary node $n$ is able to detect for each channel $c$ whether any primary user utilizes $c$. We denote $C_n \subseteq \mathcal{C}$ as the set of available channels for $n$.
...
In addition to the licenced channels there is one dedicated control channel. It is available for any secondary node $n$ to exchange control messages with any other node in its proximity (or neighborhood) $N(n)$ during the cluster formation phase. $N(n)$ is simply defined as the set of nodes located within the transmission range of $n$. Control messages involved in the clustering process can be transmitted on available licenced channels through a rendez-vous process by channel hopping~\cite{channelHopping_Rendezvous_2014, Gu_distributed_rendezvous_2014}. \textcolor{red}{(The rendez-vous should be briefly explained! TODO)} Over the control channel, a secondary user $n$ can exchange its spectrum sensing result $C_n$ to any $n' \in N(n)$.
...
\textcolor{red}{As to a pair of secondary users, when the distance between them is smaller than secondary users' transmission range, we assume the pair is able to communicate on both control channel and licensed channel, and the both are considered to be neighbours of each other. DEFINE DISTANCE! TODO for Di!}
...
As the assumed $0/1$ state \textcolor{red}{(What is that?? Define! Fix it TODO)} of connectivity is solely based on the distance between secondary users,
...
A cognitive readio network (CRN) can be represented as an undirected graph $G = (\mathcal{N}, E)$, where $E \subseteq \mathcal{N} \times \mathcal{N}$ such that $\{n_1,n_2\} \in E$  if, and only if, there exists a channel $c \in \mathcal{C}$ with $c \in C_{n_1} \cap C_{n_2}$. Note that we consider the channel availability only for \textit{one} snapshot of time. For the rest of this paper the word channel is referred to licensed channel, if the control channel is not explicitly mentioned.
...
A cluster head coordinates the activities of cluster members, \ie it notifies all the members to vacate a channel if a primary user's presence is detected by one cluster member. Further it also notifies the members to use a different common control channel for payload communication. (\textcolor{red}{(Not clear what the meaning of payload communication is. Why not omitting payload here?)}
