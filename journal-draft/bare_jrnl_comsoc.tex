...
As the transmission range of primary users is limited and secondary users are located at different locations, different secondary users may have different views of the spectrum availability, i.e., for any $n, n' \in V_N$, $C_n = C_{n'}$ does not necessarily hold.	
%
...
A cognitive radio network (CRN) can be represented as an undirected graph $G = (V_N, E)$, where $E \subseteq V_N \times V_N$ such that $\{n_1,n_2\} \in E$  if, and only if, there exists a channel $c \in \mathcal{C}$ with $c \in C_{n_1} \cap C_{n_2}$. Note that we consider the channel availability only for \textit{one} snapshot of time. For the rest of this paper the word channel is referred to licensed channel, if the control channel is not explicitly mentioned.
...
$N(n)$ & neighborhood of node $n$    \\
$C$ & a cluster -- remove ? \\
$C_i$ & a cluster whose cluster head is $i$ -- remove ? \\
& (in Section~\ref{centralized_scheme}, $C_i$ is the $i$th cluster \\
& among the legitimate clusters.) -- remove ?\\
$V_i$   & set of available channels at CR node $i$ --$V$ is usually a set of vertices \\
$V_C$   & set of available CCCs of cluster $C$ --$V$ is usually a set of vertices \\
$V_{C_i}$   & set of available CCCs of cluster $C_i$ --$V$ is usually a set of vertices\\
...
$K(i)$ & a cluster whose cluster head is $i$  \\
$C(i)$   & set of available channels at CR node $i$  \\
$C(K)$   & set of available CCCs of cluster $K$ \\
$C(K(i))$   & set of available CCCs of cluster $K(i)$ \\
$C_{K_i}$   & set of available CCCs of cluster $K(i)$ \\
$h(K)$ & cluster head of a cluster $K$\\
$h_K$ & cluster head of a cluster $K$\\
$\delta$ & desired cluster size\\
$S(i)$ & set of claiming clusters, each of which includes \\
& debatable node $i$ after phase I\\
...
$K_C$   & set of available CCCs of cluster $C$ \\
$K_{C_i}$   & set of available CCCs of cluster $C_i$ \\
$h_C$ & cluster head of a cluster $C$\\
$\delta$ & desired cluster size\\
$S_i$ & set of claiming clusters, each of which includes \\
& debatable node $i$ after phase I\\
