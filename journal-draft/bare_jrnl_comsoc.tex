...
%	\item $\mathcal{R}=\left\{1,\ldots,m\right\}$, the set of resources which player can choose, which are all the clusters in our model.
	...
	As debatable node $i$ is supposed to choose one claiming cluster in our problem, only one piece of resource is allocated for $i$.%, accordingly this congestion game is a singleton game.
...
%				& $ID_{H_C}$ and $K_C$ for $h+m^2c$ times, notification to join in one cluster for $m^2c$ times					\\ \hline
% ROSS-DFA, ROSS-x-DFA 		&   $h+ 2m$	 (upper bound) 						& $ID_{H_C}$ and $K_C$ for $h+m$ times, notification to join in one cluster for $m$ times	 					\\ \hline
