...
\thanks{Di Li was with RWTH Aachen university, Germany.}% <-this % stops a space
\thanks{Erwin Fang is with ETH, Switzerland.}% <-this % stops a space
\thanks{James Gross is with KTH Royal Institute of Technology, Sweden.}% <-this % stops a space
...
The congestion game model is adopted to analyse the process of cluster formation, which not only contributes the design of the distributed clustering scheme, but also provides the guarantee on the convergence into Nash Equilibrium and the convergence speed.
...
% in other words, their claiming clusters become clusters which are beside them. 
...
In fact, the upper bound of the number of steps which are actually involved in this process is much smaller than $N^3$.%, as both $n$ and $m$ are considerably smaller than $N$.
...
\hline
 ...
 ROSS-DGA, ROSS-$\delta$-DGA 	&$\mathcal{O}(N^3)$ (worst case)		&   $N+n^2m$ (upper bound)  				&   \multirow{2}{*}{\parbox{7.7cm}{PhaseI: ID, $d_i$,$g_i$, which are 3 bytes;\\ PhaseII: Cluster head $i$ broadcasts channel availability on all cluster members, where are $|C(i)| |\mathcal{K}|$ bytes}}								\\ \cline{1-3}
...
%
From Figure~\ref{singleton_clusters_100} to~\ref{singleton_clusters_300}, we can see that significantly less unclustered CR nodes are generated by the variants of ROSS whcih have size control mechanism.
...
The proposed clustering schemes generate clusters which have long time expectancy against the primary users' activity, besides, the proposed schemes generate clusters which have desired sizes.
...
Through simulation and theoretical analysis, we find that the distributed schemes achieve similar performance with the centralized scheme in terms of cluster robustness, signaling overhead and cluster sizes, and outperform the comparison distributed scheme on the above mentioned metrics.
