...
Considering the above mentioned requirements on the formed clusters, we present the definition of robust clustering problem as follows.
...
In the following, when we mention the variants of ROSS, we refer the above mentioned four schemes.
...
When we only consider the clusters which are not singleton, the average number of common channels of SOC drops to 2.5. 
\begin{figure}[ht]
\begin{center}
...
In the simulation, we investigate the schemes with four metrics.
...
\end{tabular}
    \begin{tablenotes}
      \item\label{tnote:robots-r1}Assuming the data structure of the clustering result is given as \{ Node ID $i$, cluster head ID h($C$) where $i\in C$, $i\in \mathcal{N}$ \}.
    \end{tablenotes}
    \end{threeparttable}
}
}
...
As distributed schemes, the variants of ROSS outperform SOC considerately on three metrics.
...
\begin{proof}
To prove the robust clustering problem is NP-hard, we reduce the NP-hard problem \textit{weighted k-set packing} to it to show the robust clustering problem is at least as hard as the weighted $k$-set packing problem.
The definition of weighted $k$-set packing problem is given as bellow.
\begin{mydef}
\label{def_kset_packing}
\textit{Weighted k-set packing.} 

Given a finite set $\mathcal{G}$ of non-negative integers where $\mathcal{G} \subsetneq \mathbb{N}$, and a collection of sets $\mathcal{Q}=\{S_1,S_2,\cdots,S_m\}$ where $S_i \subseteq \mathcal{G}$ for $1 \leq i \leq m$, besides, every set $S$ in $\mathcal{Q}$ has a weight $\omega(S) \in \mathbb{R}$.
%Further, we are given a threshold value $\lambda \in \mathbb{N}$.\\
The weighted $k$-set packing problem is to find a collection $\mathcal{S} \subseteq \mathcal{Q}$ such that $\mathcal{S}$ contains only pairwise disjoint sets and the total weight of the sets in $\mathcal{S}$ is greater than a given positive number $\lambda$, i.e., $\sum_{S \in \mathcal{S}} \omega(S) > \lambda$.
\end{mydef}

Weighted $k$-set packing is known to be NP-hard when $k\geqslant 3$~\cite{Computers_a_Intractability}.
In the proof, we will show the existence of a polynomial-time algorithm $\sigma$ which transforms any instance $\mathcal{I}$ of a weighted $k$-set packing problem, \ie a collection of sets, into a formation of clusters for the robust clustering problem.
W.l.o.g. let set $\mathcal{G} = \{ 1, \ldots , N \}$.
The polynomial algorithm $\sigma$ consists of four steps.
...
In particular, as to each set $S\in \mathcal{I}$, we duplicate every element and assign a new value to the new element, which is N bigger than the original element.
...
\item In the second step, all the elements in $\mathcal{I'}$ are mapped to CR nodes on a two-dimensional Euclidean plane.
In particular, there is a bijection between every element in $\mathcal{I'}$ to one CR node whose ID is identical with the element value.
...
Accordingly, there is also a bijection between a set in $\mathcal{I'}$ and a cluster in the CRN.
We regulate the number of CCs is equal to the weight of the corresponding set.
...
\item In this step, we transform the instance $\mathcal{I}$ to a formation of clusters for CRN.
...
We adjust the locations of the CR nodes so that one pair of CR nodes whose IDs are exact $N$ apart are neighbors to each other.
...
Because of the CR nodes which are mapped from the duplicated elements in $\mathcal{I'}$, there is no singleton cluster.
This transformation requires $2\cdot\sum_{S_i \in \mathcal{I'}} |S_i|$ steps.
%
\item Now we map the elements in $\mathcal{Q}\setminus \mathcal{I}$ to CR nodes. 
We arbitrarily put these CR nodes in the plane, and these CR nodes form single node clusters by themselves.
According to the definition of robust clustering problem in CRN, the number of CCs in these single node clusters is 0.
These singleton clusters and the clusters mapped from $\mathcal{I'}$ constitute a clustering solution, and finding these singleton clusters requires at most $N$ steps.
...
When there are only disjoint sets in an instance $\mathcal{I}$ for weighted $k$-set packing, the sum of weights is identical to the sum of the numbers of CCs in the CRN which are mapped from $\mathcal{I'}$, even $\mathcal{I}$ contains sets which only have one element.
...
In the other way around, when an instance is false for weighted $k$-set packing problem, the sum of the numbers of CCs of the clusters in the mapped CRN is smaller than $\lambda$, then the formation of the clusters is not a solution for the robust clustering problem.
...
\begin{table}[h!]
    ...
    \cmidrule(r){1-1}\cmidrule(lr){2-2}
      ...
      & 
		$\{ \, \{1,11\},\{2,12, 3, 13\}, \{ 4,14\} \, \}$
      \\    \cmidrule(r){1-1}\cmidrule(lr){2-2}
	clusters mapped from $\mathcal{I}$  (step 2 \& 3)
	  &
	  \begin{minipage}{.3\textwidth}	
      \includegraphics[width=0.6\linewidth]{np_hard_proof_step_2_3.pdf}
    \end{minipage}
          \\    \cmidrule(r){1-1}\cmidrule(lr){2-2}
	formation of clusters (step 4)
	  &
	  \begin{minipage}{.3\textwidth}	
