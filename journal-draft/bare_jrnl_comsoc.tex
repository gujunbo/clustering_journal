...
The cluster head can enable all the CR devices within the same cluster to stop payload transmission swiftly on the operating channel and to vacate the channel ~\cite{willkomm08}.
%
...
In the context of CRN, the activity of primary users is usually unknown to the secondary users, thus when the primary users' activity is deemed as random, the cluster which secures more CCs will anticipate a longer life expectancy.
...
For example, spectrum sensing accuracy correlates with the cluster size~\cite{Consensus_based_clustering12}, and power consumption doesn't favor small clusters~\cite{clustering_globecom11, EnergyEfficientClusteringRouting_2015}.
...
%is synonymous with the affluence of CCs which are possessed by that cluster, to ....
...
There has been a lot of research done on clustering in wireless networks.
...
In respect of CRN, \cite{Zhao07, Chen07,Affinity_clustering_09icccn} propose the clustering schemes which partition the CRN and make sure that every cluster possesses at least one CC.
...
\cite{cluster_EW10} forms clusters to coordinate the control channel usage within the clusters.
...
%
A distributed clustering scheme (denoted as SOC) which is designed to generate robust clusters against primary users is proposed in~\cite{LIU_TMC11_2}.
The robustness of the clusters is obtained from the multiple CCs in each cluster.
...
The new extensions are made on basis of ROSS and its light weight version, the latter involves less overheads thus are more suitable for the scenario where fast deployment is desired.
...
%As to the relation between the primary and secondary users,
...
We assume the primary users change their operation channels slowly thus we omit the time index for the spectrum availability. As the result of spectrum sensing, $K_i \subseteq \mathcal{K}$ denotes the set of available licensed channels for CR user $i$.
...
In the following, we refer to the licensed channel as channels in general, and will explicitly mention the dedicated control channel if necessary. 
...
Given a cognitive radio network $\mathcal{N}$ where $|\mathcal{N}|=N$, the collection of all the possible clusters\footnote{Possible cluster means the collection of CR nodes, which complies with the definition in~\ref{sec:model}} is denoted as $\mathcal{S}$ where $\mathcal{S}=\{C_1, C_2,\ldots,C_{|\mathcal{S}|}\}$ \footnote{The subscripts of the clusters can be decided in any convenient way, \ie the sequence of identifying them.} and there is $\bigcup_{1\leq i \leq |\mathcal{S}|} C_i = N$.
%
When the requirements on the cluster size are enforced, \ie the desired size is $\delta$ and the cluster sizes should fall in the scope $\langle\delta_1, \delta_2\rangle$, where $\delta, \delta_1, \delta_2\in \mathbb{Z}^+$, $\delta_1 \leq \delta\leq \delta_2$, then a feasible clustering solution is a subcollection $\mathcal{S}' \subseteq \mathcal{S}$, which satisfies 
$\delta_1\leq|C_i|\leq \delta_2, \bigcup_{C_i\in \mathcal{S}'} C_i = \mathcal{N}$ and $C_{i'}\cap C_i =\emptyset$, $C_{i'}, C_i\in \mathcal{S}'$, $i'\neq i$.

The robust clustering problem in CRN is to find the optimal clustering solution from the feasible clustering solutions, whose sum of the numbers of CCs of clusters $\sum_{C\in \mathcal{S}'} f(C)$ is the maximal.
...
\begin{theorem}
\label{theorem1}
Robust clustering problem in CRN is NP-hard, when the maximum size of clusters is larger than 3, $\delta_1=1$ and $\delta_2 = N$.
...
We propose a centralized solution for the robust clustering problem, which solves an optimization with standard solvers. 
To formulate the optimization, we need to do some preparation beforehand.
First, all the possible clusters complying with the description in the system model are found and constitute a set $\mathcal{S}$.
...
Considering $|\mathcal{S}|=M$, $C_i\in \mathcal{S}$ means the $i$ th cluster in $\mathcal{S}$, and $\delta$ is the desired size, the size related weight for each cluster is given as follows,
$$
...
\end{array} \right.
$$
where $\rho_1, \rho_2, \cdots$ are positive values.
In particular, $\rho$ increases with the divergence between $|C_i|$ and $\delta$, \ie these is $\rho_2> \rho_1>0$.

 
The optimization searches the set $\mathcal{S}$ and decides on certain clusters which constitute the whole CRN without overlapping between any two of them, besides, the sum of CCs of the chosen clusters is maximized.
...
$w_i$ and $x_{ij}$ are the two binary variables in this problem.
...
Being either 1 or 0, $w_i$ denotes whether the $i$th cluster $C_i$ in $\mathcal{S}$ is chosen by the solution or not.
...
$t_{ij}$ is a constant which is $\frac{q_{ij}}{|C_i|} - p_i(C_i)$.
...
$p_i(C_i)$ is the size related weight which has been introduced.
...
Thus the second item discourages the appearance the clusters whose sizes are different from $\delta$, especially those whose sizes are not $\delta$.
...
The second constraint regulates that when the $i$th possible cluster $C_i$ is chosen, there will be exactly $|C_i|$ CR nodes residing in $C_i$.
...
In the worst case, \ie the CRN forms a full connected graph, the size of $S$ is $\Sigma_{r=1}^{N}\ {N \choose r} = 2^N-1$.
...
Another obstacle to apply this centralized scheme is, the centralized entity firstly needs to collect the information from all the CR nodes, then computes the clustering solution and distributes it across the whole network.
...
We have the following theorem to show that when the secondary users' individual connectivity degrees are greater than zero, every secondary user will eventually be either integrated into a certain cluster, or becomes a cluster head.
\begin{theorem}
...
During Phase I, whenever ever a CR node is decided to be a cluster head and accordingly forms a cluster, or its cluster's composition is changed, the cluster head will broadcast the updated information about its cluster, which includes the sets of available channels on all its cluster members.
...
Note that node $i$ has been notified of the spectrum availability on all the cluster members of each claiming cluster, thus node $i$ is able to calculate how many more CCs can be produced in one claiming cluster if $i$ leaves that cluster.
%Based on this calculation, $i$ decides on one claiming cluster to stay and leaves the other claiming clusters.
If there exists one cluster $C\in S_i$, when $i$ leaves this cluster brings less increased CCs than leaving any other claiming clusters, then $i$ chooses to stay in cluster $C$.
...
From the new perspective, the debatable nodes are regarded as isolated and don't belong to any cluster, in other words, their claiming clusters become clusters which are beside them. 
...
The number of clusters heads, as mentioned in Section~\ref{phaseI}, is dependent on the network density and the CR node's transmission range.
...
As to the CRN shown in Figure~\ref{fig1}, the resulted clusters by the centralized scheme and SOC are shown in Figure~\ref{fig:final_clustering}.
...
First, the unclustered CR nodes (synonym of singleton clusters) should not be considered when obtaining the average number of CCs, as singleton clusters doesn't contribute to the collaborative computing.
Second, the average number of CCs doesn't necessarily indicate the robustness of individual clusters, because the ability for a cluster to sustain also depends on the cluster size and the locations of the cluster members, and these information is not illustrated in the average number of CCs.
...
The constant for size control is 1.3.
As to the centralized scheme, the parameters used in the \textit{punishment} for choosing the clusters with undesired sizes are set as follows, $\rho_1 =  0.4$, $\rho_2 =  0.6$.
...
In the other hand, SOC generates the most unclustered CRs.
...
\hline
 ...
 ROSS-DGA, ROSS-$\delta$-DGA 	&$\mathcal{O}(N^3)$ (worst case)		&   $h+2m^2d$ (upper bound)  				&   \multirow{2}{*}{\parbox{7.7cm}{Cluster head $i$ broadcasts channel availability on all cluster members ($|C(i)| |\mathcal{K}|$ bytes); Cluster member $i$ broadcasts the new individual connectivity $d_i$ after being included in one or more clusters (1 byte)}}								\\ \cline{1-3}
 ...
 \\ \hline
 Centralized			&$\mathcal{O}(N)$			&	$h + m + N$ (upper bound)~\cite{Efficient_broadcasting_gathering_adhoc}		& clustering result (2$N$ bytes) \tnotex{tnote:robots-r1} 					\\ \hline
\end{tabular}
    \begin{tablenotes}
...
%\item In the first step, we transform the instance $\mathcal{I}$ to $\mathcal{I'}$ by duplicating the elements of the sets in $\mathcal{I}$.
%In particular, as to each element in set $S\in \mathcal{I}$, we duplicate the element and assign a new index which is N bigger than the original index.
%The weight of these sets remain unchanged.
%The purpose of this transformation is to eliminate the set in $\mathcal{I}$, which has only one element.
...
%The channel availability on the CR nodes whose IDs are $N$ apart is the same.
%Besides, there is a bijection between every set in $\mathcal{I'}$ and one cluster in the CRN.
%In this process, we decide the channel availability on each CR node so that the number of CCs in each cluster is equal to the weight of the corresponding set.
%
%When there are two sets in $\mathcal{I'}$ and one is a subset of the other one, \ie  $S_1\subset S_2$, and $\omega(S_1) < \omega(S_2)$, then we should swap the weights of the sets before mapping them into clusters. 


%\item In second step, we map the elements in $\mathcal{Q}\setminus \mathcal{I}$ to CR nodes. 
%We arbitrarily put these CR nodes in the CRN, then these CR nodes form singleton clusters by themselves.
%According to the definition of robust clustering problem in CRN, the number of CCs of these singleton clusters is 0.
%These singleton clusters and the clusters mapped from $\mathcal{I'}$ constitute a clustering solution.

% finding these singleton clusters requires at most $N$ steps.
...
%We have found the polynomial algorithm $\sigma$ which transforms an instance of weighted $k$-set packing to an instance of the robust clustering problem in CRN.
...
%    \cmidrule(r){1-1}\cmidrule(lr){2-2}     
%      Instance for Weighted $k$-set packing  & $\{ \{1\}, \{2,3\}, \{4\} \}$, weights are 2, 3, 4\\ 
%    \cmidrule(r){1-1}\cmidrule(lr){2-2}
%      Instance with dummy elements (step 1)
%      & 
%		$\{ \, \{1,11\},\{2,12, 3, 13\}, \{ 4,14\} \, \}$
%      \\    \cmidrule(r){1-1}\cmidrule(lr){2-2}
%	clusters mapped from $\mathcal{I}$  (step 2)
%	  &
%	  \begin{minipage}{.3\textwidth}	
%      \includegraphics[width=0.6\linewidth]{np_hard_proof_step_2_3.pdf}
%    \end{minipage}
%          \\    \cmidrule(r){1-1}\cmidrule(lr){2-2}
%	formation of clusters (step 3)
%	  &
%	  \begin{minipage}{.3\textwidth}	
%      \includegraphics[width=0.6\linewidth]{np_hard_proof_step4.pdf}
%    \end{minipage}
%	  \\       \bottomrule
%      \end{tabular}
%      \caption{}
