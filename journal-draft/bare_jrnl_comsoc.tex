...
All the possible clusters complying with the Description in the system model are found and constitute a set $\mathcal{S}$.
Besides, we assign a weight to each cluster, which correlates with the difference between the cluster size and the desired cluster size. 
In particular, considering $|\mathcal{S}|=M$ and $C_i\in \mathcal{S}$ means the $i$ th cluster in $\mathcal{S}$, and $\delta$ is the desired size, then the weight for each cluster is given as follows,
$$
