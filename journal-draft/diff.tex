...
%DIF DEL ettdoc_0705.tex   Mon Sep 11 21:54:48 2017
%DIF ADD ettdoc.tex        Thu Sep 28 19:12:15 2017
...
\author{Di Li\textsuperscript{1}\DIFaddbegin \corrauth\DIFaddend , Erwin Fang\textsuperscript{2}, James Gross\textsuperscript{3}\DIFdelbegin %DIFDELCMD < \corrauth%%%
\DIFdelend }
...
\DIFdelbegin %DIFDELCMD < \corraddr{Communication Theory Lab
%DIFDELCMD < School of Electrical Engineering, 
%DIFDELCMD < KTH Royal Institute of Technology
%DIFDELCMD < SE - 100 44 Stockholm\\Email: james.gross@ee.kth.se}
%DIFDELCMD < %%%
\DIFdelend %DIF > \corraddr{Communication Theory Lab
%DIF > School of Electrical Engineering, 
%DIF > KTH Royal Institute of Technology
%DIF > SE - 100 44 Stockholm\\Email: james.gross@ee.kth.se}
\DIFaddbegin \corraddr{Chair of Communication and Distributed Systems
...
Unlicensed channels, which are \DIFaddbegin \DIFadd{temporally }\DIFaddend available for a group of cognitive radio users \DIFaddbegin \DIFadd{in one area}\DIFaddend , consolidate the group into a cluster\DIFdelbegin \DIFdel{and the number of the }\DIFdelend \DIFaddbegin \DIFadd{.
More }\DIFaddend available unlicensed channels \DIFdelbegin \DIFdel{decides that cluster 's robustness }\DIFdelend \DIFaddbegin \DIFadd{in a cluster make the cluster more likely to uphold }\DIFaddend against the licensed users' influence \DIFaddbegin \DIFadd{than only one available channel, which make that cluster to be more robust}\DIFaddend .
This paper analyses the problem of how to form robust clusters in a cognitive radio network so that more cognitive radio users can \DIFdelbegin \DIFdel{get the benefits }\DIFdelend \DIFaddbegin \DIFadd{still be benefited }\DIFaddend from cluster structure when the primary users' operations becomes more intense.
...
\DIFdelbegin \DIFdel{Efficient spectrum sensing can be achieved }\DIFdelend \DIFaddbegin \DIFadd{The rate of false negative, \ie misdetecting the active primary users, can be decreased }\DIFaddend by cooperative spectrum sensing of multiple secondary users, which has been shown to cope effectively with noise uncertainty and channel fading, thus remarkably improving the sensing accuracy~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{coorperativeSensing_Akyildiz11}}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Jacob2012, coorperativeSensing_Akyildiz11}}%DIFAUXCMD
.
%DIF > C3
}\DIFaddend Collaborative sensing relies on the consensus of CR users\footnote{The terms user and node appear interchangeably in this paper. In particular, user is adopted when its networking or cognitive ability are discussed or stressed, while we refer to nodes typicallly in the context of the network topology.} within a certain area, in this regard, clustering is regarded as an effective method to realize cooperative spectrum sensing~\cite{Sun07_clustering_spectrum_secsing}.
% ... decreases considerably the negative probability due to the random channel effects like fading and shadowing.
\DIFdelbegin \DIFdel{Clustering is a }\DIFdelend \DIFaddbegin 

\DIFadd{Clustering is the }\DIFaddend process of grouping certain users in geographic proximity into a collective\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, and as an effective technique, it is proposed with the emergence of mobile wireless networks~\mbox{%DIFAUXCMD
\cite{gerla_97} }%DIFAUXCMD
and there has been a plethora of clustering schemes.
As to the wireless networks where no restriction is imposed on the nodes to access the spectrum, \ie 
%DIF > Different clustering strategies have been proposed for wireless networks of different types.
wireless ad-hoc, mesh networks and sensor networks, the objective of clustering can be to decrease the transmission power consumption~\mbox{%DIFAUXCMD
\cite{Kawadia03}}%DIFAUXCMD
, to improve the routing performance~\mbox{%DIFAUXCMD
\cite{clustering_mesh_globecom2010}}%DIFAUXCMD
, or to imrove the network lifetime and coverage~\mbox{%DIFAUXCMD
\cite{Abbasi_survey_07}}%DIFAUXCMD
.
%DIF > In CRN, clusters are formed in the very beginning of the network operation, and re-formed periodically according to the dynamics of the CRN.
}\DIFaddend In the context of cognitive radio netowrks, a formed cluster helps the participants not only \DIFdelbegin \DIFdel{to }\DIFdelend improve the spectrum sensing accuracy, but also \DIFdelbegin \DIFdel{to }\DIFdelend coordinate the channel switch operation when the primary users are detected by at least one secondary user in that cluster. Then all the cluster menmbers can stop payload transmission swiftly and vacate the operating channel ~\cite{willkomm08}.
...
\DIFdelbegin \DIFdel{Except for benefiting the dynamic spectrum sharing in CRN, clustering contributes to the other wireless networks in many aspects, which has already been discussed in many works~\mbox{%DIFAUXCMD
\cite{Kawadia03, clustering_mesh_globecom2010, Abbasi_survey_07}}%DIFAUXCMD
.
}\DIFdelend %DIF > Except for benefiting the dynamic spectrum sharing in CRN, clustering contributes to the other wireless networks in many aspects~\cite{Kawadia03, clustering_mesh_globecom2010, Abbasi_survey_07}.

%DIF < %[crn for clustering] 
%DIF < %due to attenuation of signal propagation, primary users can only be detected by CR users when they locate closely to CR users.
%DIF < In cognitive radio networks, secondary users which locate closely with each other are possibly affected by the same group of primary users, so that the availability of licensed spectrum is similar to them, \ie certain channels are available on each of them.
%DIF < The similarity of available spectrum on a group of neighbouring CR nodes, along with the benefit of collaborative decision among multiple nodes, leads to clustering as an effective approach for many applications.
%DIF < [robustness issue for clustering]
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In CRN, \DIFdelbegin \DIFdel{clusters are formed in the very beginning of the network operation, and re-formed periodically according to the dynamics of the CRN.
Each formed cluster has }\DIFdelend \DIFaddbegin \DIFadd{a formed cluster seizes }\DIFaddend one or multiple unlicensed channels which are available for every CR node in the cluster.
...
\DIFdelbegin \DIFdel{Both }\DIFdelend \DIFaddbegin \DIFadd{The availability of CCs within a cluster decides the existence of that cluster, \ie if no CCs are available then the corresponding cluster does not exist.
...
%DIF > 
Within in a cluster, both }\DIFaddend payload and control overheads can be transmitted on the CCs.
%DIF < Out of the available CCs, there are always one or multiple channels which are used for the payload communication.
When one or several cluster members can not access \DIFdelbegin \DIFdel{one }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend certain CC on which primary \DIFdelbegin \DIFdel{some user}\DIFdelend \DIFaddbegin \DIFadd{user's }\DIFaddend activity is detected, the channel will be excluded from the set of CCs.
In particular, if that channel is being used for payload communication, the communication pair will stop and resume \DIFdelbegin \DIFdel{the }\DIFdelend transmission on another available CC.
\DIFdelbegin \DIFdel{The availability of CCs within a cluster defines the existence of that cluster, \ie if no CCs are available then the corresponding cluster does not exist.
}\DIFdelend %As long as there is a CC available, the corresponding cluster can continue to exchange payload data.
\DIFdelbegin \DIFdel{In the context of CRN, the activity of primary users is usually unknown to the secondary users. In such cases the primary users' activity is deemed as random.
Consequently, a cluster which secures more CCs will anticipate a longer life expectancy.
In other words, such clusters are more robust.
It }\DIFdelend \DIFaddbegin 


\DIFadd{Cluster size is inherently correlated with the cluster robustness.
Given a CRN, it }\DIFaddend is obvious that \DIFdelbegin \DIFdel{fewer secondary users in one cluster }\DIFdelend \DIFaddbegin \DIFadd{getting rid of certain secondary users from a cluster may }\DIFaddend yield more CCs.
...
\DIFdelbegin \DIFdel{As }\DIFdelend \DIFaddbegin \DIFadd{On the other hand, as }\DIFaddend to the cluster which consists of many nodes, although the spectrum sensing is benefited, it usually leads to less common channels. This undermines the robustness of the cluster.
...
\DIFaddend In this paper \DIFdelbegin \DIFdel{we will analyze the robust clustering problem and propose efficient solutions.
...
%DIF < In this paper we will introduce a robust cluster structure for CRN, where the robustness means the ability of the clusters to sustain under the influence of the increasing activity of primary users, and the direct metric is the number of CCs in the clusters.
}\DIFdelend \DIFaddbegin \DIFadd{robust clustering in CRN is analyzed as a mathematical problem and proved to be NP-hard, then both centralized and distributed schemes are proposed.
We rethink the metric used to measure the cluster robustness in the previous works~\mbox{%DIFAUXCMD
\cite{Li11_ROSS} }%DIFAUXCMD
and~\mbox{%DIFAUXCMD
\cite{LIU_TMC11_2}}%DIFAUXCMD
, and we claim cluster robustness can not be indicated merely by the average number of CCs of clusters, but by the ability of the clusters to uphold when the primary users' activity becomes more active.
...
The propsoed distributed schemes extend our previous work ROSS (Robust Spectrum Sharing)~\mbox{%DIFAUXCMD
\cite{Li11_ROSS} }%DIFAUXCMD
by involving cluster size control.
Our distributed schemes solve the size devergence problem in clusters as appears in~\mbox{%DIFAUXCMD
\cite{Li11_ROSS} }%DIFAUXCMD
and~\mbox{%DIFAUXCMD
\cite{LIU_TMC11_2}}%DIFAUXCMD
.
Besides, our proposed distributed schemes are still suitable for the scenario where fast deployment is desired.
Throughout this paper, we call the proposed distributed schemes the }\textit{\DIFadd{variants of ROSS}}\DIFadd{.
 }\DIFaddend 

%DIF > 
%DIF > 
\DIFaddbegin \DIFadd{The rest of paper is organized as follows. 
In section~\ref{related_work}, we will introduce the clustering techniques in CRN and those focusing on robustness.
The relation between this paper and work~\mbox{%DIFAUXCMD
\cite{Li11_ROSS} }%DIFAUXCMD
is elaborated.
...
%DIF > The clustering problem is given through analysis and a centralized scheme is proposed in section~\ref{centralized_scheme}.
...
\label{related_work}
\DIFadd{In the following we will give a brief review on the clustering in CRN, with emphasis on the theme of robust clustering.
%DIF > %[crn for clustering] 
%DIF > %due to attenuation of signal propagation, primary users can only be detected by CR users when they locate closely to CR users.
%DIF > In cognitive radio networks, secondary users which locate closely with each other are possibly affected by the same group of primary users, so that the availability of licensed spectrum is similar to them, \ie certain channels are available on each of them.
%DIF > The similarity of available spectrum on a group of neighbouring CR nodes, along with the benefit of collaborative decision among multiple nodes, leads to clustering as an effective approach for many applications.
%DIF > [robustness issue for clustering]
%DIF > %%%%%%%%%%
}\DIFaddend %On the other hand, large clusters are not preferred in some scenarios neither, \eg for the CRN composed with resource limited users, managing the cluster members in a large cluster is a substantial burden. 
...
%DIFDELCMD < %%%
\DIFdel{Different clustering strategies have been proposed for wireless networks of different types.
As there is no restriction on the nodes to access the spectrum in wireless ad-hoc, mesh networks and sensor networks, the objective of clustering is to decrease the transmission power consumption~\mbox{%DIFAUXCMD
\cite{Kawadia03}}%DIFAUXCMD
, to improve the routing performance ~\mbox{%DIFAUXCMD
\cite{clustering_mesh_globecom2010}}%DIFAUXCMD
, or to imrove the network lifetime and coverage~\mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{The authors of~\mbox{%DIFAUXCMD
\cite{Mansoor2015} }%DIFAUXCMD
propose a clustering algorithm which aims to speed up the process of re-clustering in case a cluster doesn't uphold in from of active primary users.
...
}\DIFaddend Robustness of clusters is discussed in~\cite{LIU_TMC11_2}. A distributed clustering scheme (denoted as SOC) which is designed to generate robust clusters against primary users is proposed.
...
%
A distributed clustering scheme ROSS \DIFaddbegin \DIFadd{(Robust Spectrum Sharing) }\DIFaddend is proposed in~\cite{Li11_ROSS} under the game theoretic framework. 
...
Furthermore, \DIFdelbegin \DIFdel{this work doesn}\DIFdelend \DIFaddbegin \DIFadd{both this work and SOC define robustness just to be the number of CCs per cluster, and don}\DIFaddend 't consider the \DIFdelbegin \DIFdel{robustness }\DIFdelend \DIFaddbegin \DIFadd{sustainability }\DIFaddend of clusters against the increasing activity of primary users, which leaves their claim \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend robustness unverified.
%
\DIFdelbegin \DIFdel{This paper extends~\mbox{%DIFAUXCMD
\cite{Li11_ROSS} }%DIFAUXCMD
in two dimensions.
...
Besides, this involment of cluster size enable us to solve the problem of clusters size devergence, which appears in~\mbox{%DIFAUXCMD
\cite{Li11_ROSS} }%DIFAUXCMD
and~\mbox{%DIFAUXCMD
\cite{LIU_TMC11_2}}%DIFAUXCMD
.
...
%DIF < We stick to the motivation of forming robust clusters in CRN \ie let more CR users benefited from the cooperative decision making which is due to the clusters. 
%DIF < We propose both centralized and distributed clustering schemes, which result in more CR users in the clusters composed with multiple CR users, not only when the clusters are just formed, but also afterwards when the primary users' activity changes.
%DIF < Besides, both centralized and distributed schemes take the cluster size into consideration, and both can control the sizes of the formed clusters to certain extent.
%DIF < In particular, the decentralized clustering approach ROSS (RObust Spectrum Sharing) is able to form clusters with desired sizes.
%DIF < When compared with other distributed works, ROSS involves smaller signaling overhead and the generated clusters are significantly more robust against the primary users which appear after the clusters are formed.
%DIF < thesis
%DIF < and the generated clusters are more robust than the other clustering scheme which has claims on cluster robustness, \ie more secondary users residing in clusters against increasing influence from primary users.
%DIF < 
%DIF < ROSS selects cluster head through coordination within its neighborhood, and then cluster membership is decided locally and its fast convergence is proved under game theoretic framework. 
%DIF < For our scheme we can prove convergence in cluster formation phase and resolve ambiguities with respect to cluster membership in a game-theoretic setting. 
...
Throughout this paper, we refer to the clustering schemes on the basis of ROSS as the }\textit{\DIFdel{variants of ROSS}}%DIFAUXCMD
\DIFdel{.
}\DIFdelend 
...
\DIFdelbegin \DIFdel{The rest of paper is organized as follows. 
...
%DIFDELCMD < %%%
\DIFdelend \section{System Model and Problem Formulation}
\label{sec:model}
...
\DIFdelbegin \DIFdel{A cluster can }\DIFdelend \DIFaddbegin \DIFadd{Our scheme enables the cluster heads are selected in a distributed manner, and a cluster can only }\DIFaddend be formed only by \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{these selected }\DIFaddend cluster head.
%DIF > C1-4
...
As introduced in Section~\ref{intro}, \DIFdelbegin \DIFdel{in order to be robust against primary users' activity, the formed clusters should }\DIFdelend \DIFaddbegin \DIFadd{robustness of the clusters is their ability to uphold with the influence of the active primary users, and it is represented by the number of secondary users which are not included in any cluster.
To achieve better robustness, we form the clusters so that they }\DIFaddend have more CCs.
\DIFdelbegin \DIFdel{On the other hand}\DIFdelend \DIFaddbegin \DIFadd{Meanwhile}\DIFaddend , the sizes of the formed clusters should be regulated, \ie they don't diverge from a given value greatly.\DIFaddbegin \DIFadd{adv
}\DIFaddend %Considering the above mentioned requirements, we present the definition of robust clustering problem as follows.
...
\section{Distributed Clustering Algorithm: \DIFaddbegin \DIFadd{Variants of }\DIFaddend ROSS}
\label{ross}

In this section we introduce \DIFdelbegin \DIFdel{the distributed clustering scheme ROSS.
With }\DIFdelend \DIFaddbegin \DIFadd{our distributed clustering schemes.
With the variants of }\DIFaddend ROSS, CR nodes form clusters based on the proximity of the available spectrum in their neighborhood after a series of interactions with their neighbors.
\DIFdelbegin \DIFdel{ROSS consists }\DIFdelend \DIFaddbegin \DIFadd{The variants of ROSS consist }\DIFaddend of two cascaded phases: \textit{cluster formation} and \textit{membership clarification} \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{as shown in Figure~\ref{flowChartROSS}.
}\begin{figure}[ht!]
  \centering
\includegraphics[width=0.8\linewidth]{flow_chart.pdf}
	\caption{\DIFaddFL{Processors of ROSS}}
	\label{flowChartROSS}
\end{figure}

\DIFaddend In the first phase, clusters are formed quickly and every CR user becomes either \DIFdelbegin \DIFdel{a }\DIFdelend cluster head or \DIFdelbegin \DIFdel{a }\DIFdelend cluster member, besides, cluster size control is implemented in this phase.
...
%DIF < It is an indicator of node $i$'s adhesion to the CRN. 
...
\end{itemize}
Individual connectivity degree $d_i$ and neighborhood connectivity degree $g_i$ together form the \textit{connectivity vector} \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{$(d_i, g_i)$.
Connectivity vector is calculated by every secondary user after channel availability is obtained, and is then broadcasted.
}\DIFaddend Figure~\ref{fig1} illustrates an \DIFdelbegin \DIFdel{example }\DIFdelend CRN where every node's \DIFaddbegin \DIFadd{channel availability and }\DIFaddend connectivity vector is shown.
\DIFaddbegin \DIFadd{This CRN is used as an example in the rest of this section to illustrate the proposed distributed schemes.
%DIF > In Figure~\ref{fig1}, some primary users are randomly deployed in the network, and they are free to choose one channel out of the set of licensed channels $\mathcal{K}$ to operate.
%DIF > As to the secondary users, the channels they are allowed to use are those which are not ruled out by the primary users in their vicinities. 
...
% \includegraphics{figure1.pdf}
	\caption{Connectivity graph of the example CRN and the connectivity vector $(d_i, g_i)$ for each node. \DIFaddbeginFL \DIFaddFL{Primary users are not shown. }\DIFaddendFL The desired cluster size $\delta =3$. \DIFdelbeginFL \DIFdelFL{The sets of the indices of the available channels sensed by each node are: $K_A=\{1,2,3,4,5,6,10\}, K_B=\{1,2,3,5,7\}, K_C=\{1,3,4,10\}, K_D=\{1,2,3,5\}, K_E=\{2,3,5,7\}, K_F=\{2,4,5,6,7\}, K_G=\{1,2,3,4,8\}, K_H=\{1,2,5,8\}$. }\DIFdelendFL Dashed edge indicates the end nodes are within each other's transmission range.}
	\label{fig1}
\end{figure}
...
The procedure of determining \DIFdelbegin \DIFdel{the }\DIFdelend cluster heads is as follows.
...
If there is another CR node $j$ in its neighborhood, which has the same individual connectivity degree as $i$, \ie $d_j = d_i$ and $d_j < d_{k}, \forall k\in \text{Nb}(j)\setminus \{\Lambda\cup i\}$ where $\Lambda$ denotes \DIFdelbegin \DIFdel{the }\DIFdelend cluster heads, then the node \DIFdelbegin \DIFdel{between $i$ and $j$, which has }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend higher neighborhood connectivity degree will become \DIFdelbegin \DIFdel{the }\DIFdelend cluster head.
If $g_i = g_j$ as well, the node ID is used to break the tie, \ie the one with smaller node ID becomes the cluster head.
%
The node which is identified as a cluster head broadcasts a message to notify its neighbors of this change, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{then }\DIFaddend its neighbors which are not cluster heads become cluster members\DIFaddbegin \DIFadd{.
}\DIFaddend \footnote{The reason for the occurrence of the cluster heads in the neighborhood of a new cluster head will be explained in Section~\ref{ross_p1_guarantee_ccc} and \ref{ross_p2_cluster_pruning})}
\DIFaddbegin \DIFadd{Actually during the whole phase I, whenever a CR node is decided to be a cluster head thus accordingly forms a cluster, or its cluster's composition is changed, the cluster head broadcasts the new information about its cluster, which includes the sets of available channels on all its cluster members}\DIFaddend .
The pseudo code for the cluster head decision and the initial cluster formation is shown in Algorithm~\ref{alg0} in \DIFdelbegin \DIFdel{the }\DIFdelend appendix.
...
\DIFdelbegin \DIFdel{We have the following theorem to show that every secondary user will eventually be either integrated into a certain cluster or becomes a cluster head.
}%DIFDELCMD < \begin{theorem}
%DIFDELCMD < \label{clustering:theorem}
%DIFDELCMD < %%%
\DIFdel{Given a CRN, it takes at most $N$ steps that every secondary user either becomes cluster head, or gets included into at least one cluster.
}%DIFDELCMD < \end{theorem}
%DIFDELCMD < %%%
\DIFdel{Here, by }\textit{\DIFdel{step}} %DIFAUXCMD
\DIFdel{we mean one secondary user executing Algorithm~\ref{clustering:theorem} for one time.
...
%DIF < According to Theorem~\ref{clustering:theorem}, Phase I completes within a reasonable amount of time.
%DIF < 
%DIF < This judgement is conducted periodically, and phase I ends after every node ascertains it is cluster head or not.
...
As \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the nodes which are eliminated from \DIFdelbegin \DIFdel{the previous clusters}\DIFdelend \DIFaddbegin \DIFadd{a cluster}\DIFaddend , they restore their original individual connectivity degrees, then execute Algorithm~\ref{alg0} and become either cluster heads or get included into other clusters afterwards according to Theorem~\ref{clustering:theorem}.




\DIFdelbegin \DIFdel{During Phase I, whenever a CR node is decided to be a cluster head and accordingly forms a cluster, or its cluster's composition is changed, the cluster head will broadcast the updated information about its cluster, which includes the sets of available channels on all its cluster members.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsubsection{Cluster Size Control in Dense CRN}
...
Hence, a cluster head excludes some cluster members when the cluster size exceeds \DIFaddbegin \DIFadd{a certain threshold.
We set the threshold as }\DIFaddend $t\cdot \delta$, where \DIFaddbegin \DIFadd{$\delta$ is the desired cluster size.
The }\DIFaddend constant parameter $t$ is \DIFaddbegin \DIFadd{usually larger than 1 and is }\DIFaddend dependent on the network density and CR nodes' transmission range\DIFdelbegin \DIFdel{and $t>1$.
}\DIFdelend \DIFaddbegin \DIFadd{, in particular, $t$ is between 1 and the ratio between the average neighborhood size and the desired size.
%DIF > A smaller or bigger $t$ may lead to more clusters with smaller or larger size than $\delta$.
...
\DIFadd{Because of $t$, the threshold to prune is larger than the desired size, then there will be some nodes choosing to affiliate with other clusters in the following phases.
%DIF > C1-7
}\DIFaddend In particular, the cluster head removes the cluster members sequentially according to the following principle, the absence of one cluster member leads to the maximum increase of the CCs within the cluster.
...
\DIFdelbegin \DIFdel{The $t$ is set to 1.3. 	
}\DIFdelend %DIF > The $t$ is set to 1.3. 	
%DIF > C1-7
...
%DIF > %%%%%%%%%%%%%%
\DIFaddbegin \DIFadd{We have the following lemma to show that every secondary user will eventually be either integrated into a certain cluster or becomes a cluster head.
}

\begin{lemma}
\label{clustering:lemma1}
\DIFadd{Given a CRN where any secondary user is able to communicate with any other secondary user through the other nodes, then after the phase of cluster head selection and initial cluster formation, every secondary user either becomes cluster head, or gets included into at least one cluster.
}\end{lemma}

%DIF > Here, by \textit{step} we mean Algorithm~\ref{clustering:theorem} is executed by a secondary user for one time.
%DIF > In one special scenario, \eg when a cluster head is elected and all the other nodes are within its neighborhood, the number of steps is 1.
\DIFadd{The Proof is given in Appendix~\ref{proof_clustering:lemma1}.
%DIF > The proof also illustrates the maximal time needed to conduct Algorithm~\ref{clustering:theorem}. 
%DIF > Consider an extreme scenario, where all the secondary nodes sequentially execute Algorithm~\ref{alg0}, \ie they constitute a chain as discussed in the example in the proof.
%DIF > 
%DIF > Assuming there is a token in the CRN, one node is allowed to execute Algorithm~\ref{alg0} only when it holds the token, and it then transfers the token to the neighboring node on the chain.
%DIF > If one step can be finished within a certain time span $T$, then the total time needed for the network to conduct Algorithm~\ref{clustering:theorem} is $N*T$.
%DIF > C1-6
}

\begin{lemma}
\label{clustering:lemma2}
\DIFadd{When a secondary user becomes cluster head, it will not become cluster member again.
}\end{lemma}
\begin{proof}
\DIFadd{A secondary node becomes cluster head when its }\textit{\DIFadd{Individual connectivity degree}} \DIFadd{is smaller than all its neighbors.
Afterwards, the }\textit{\DIFadd{Individual connectivity degree}} \DIFadd{of its neighbors becomes $M$ and has no means to decrease, thus the cluster head will remain to be cluster head.
As to the node which is excluded form a cluster due to guaranteeing CC or size control, its }\textit{\DIFadd{Individual connectivity degree}} \DIFadd{is restored, but it will not include its former cluster head, even if it becomes cluster head as regulated in~\ref{alg0}.
}\end{proof}
...
\DIFadd{In the process of cluster head selection and initial cluster formation, the maximum number of times that a secondary node becomes cluster head is $N$.
}\end{lemma}
\DIFadd{This lemma can be easily gotten from~\ref{clustering:lemma2} considering the $N$ is the number of all the secondary users in the CRN.
Based on these Lemmas, we can easily obtain Theorem~\ref{clustering:lemma1},
}\begin{theorem}
\label{clustering:theorem}
\DIFadd{Assuming the time for a secondary user to update the information about cluster heads in its neighborhood is $T$, then it takes at most $N*T $ time to finish the process of cluster head selection and initial cluster formation.
}\end{theorem}
\DIFadd{Phase I ends when no more secondary users becoming cluster head.
based on~\ref{clustering:lemma1} and~\ref{clustering:lemma3}, the theorem is proved.
}


\DIFadd{As Algorithm~\ref{alg0} is executed concurrently by different secondary users, the needed time can be considerably reduced.
If we apply Algorithm~\ref{alg0} to the example shown in Figure~\ref{fig1}, the outcome is shown in Figure~\ref{fig2}.
Node $B$ and $H$ have the same individual connectivity degree, i.e., $d_B=d_H$. As $g_H=2>g_B=1$, node $H$ becomes the cluster head and cluster $C(H)$ is $\{H, B, A, G\}$.
}


\DIFaddend \subsection{Phase II - Membership Clarification}
...
\DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{On }\DIFaddend the other hand, the number of clusters heads is dependent on the network density and the CR node's transmission range as mentioned in Section~\ref{phaseI}.
...
Now we apply both ROSS-DGA and ROSS-DFA to the \DIFdelbegin \DIFdel{toy }\DIFdelend network in Figure~\ref{fig2} which has been applied the phase I of ROSS.
%DIF > C1-8
...
We investigate the schemes \DIFdelbegin \DIFdel{with respect to four metrics}\DIFdelend \DIFaddbegin \DIFadd{in the following aspects}\DIFaddend .
...
\item \DIFdelbegin \textbf{\DIFdel{Cluster sizes.}}
%DIFAUXCMD
%DIF < Specific clusters size is pursued in many applications due to energy preservation and the system design ~\cite{clustering_globecom11}.
\DIFdel{We investigate the distribution of CRs residing in the formed clusters with different sizes.
}%DIFDELCMD < 

%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend \textbf{Robustness of the clusters against newly added PUs.}
\DIFdelbegin \DIFdel{We increase }\DIFdelend \DIFaddbegin \DIFadd{Robustness is illustrated by }\DIFaddend the number of \DIFdelbegin \DIFdel{PUs to challenge the non-singleton clusters, and count the number of the unclustered CR nodes}\DIFdelend \DIFaddbegin \DIFadd{the unclustered CR nodes in the CRN, after the CRN being challenged by the increasing number of PUs}\DIFaddend .
...
\item \DIFaddbegin \textbf{\DIFadd{Cluster sizes.}}
%DIF > Specific clusters size is pursued in many applications due to energy preservation and the system design ~\cite{clustering_globecom11}.
\DIFadd{We investigate the distribution of CRs residing in the formed clusters with different sizes.
}



\item \DIFaddend \textbf{Amount of control messages involved.}
We investigate the number of control messages involved in the clustering process.

\DIFaddbegin \item \textbf{\DIFadd{Influence from inaccurate spectrum sensing.}}
\DIFadd{The above simulations are conducted with the assumption of perfect spectrum sensing.
...
%DIF > The errors in spectrum sensing is inevitable, and the decision is made by the secondary users and based on the sensing results, without considering the difference between the sensing result and the ground truth.
...
\DIFadd{We assume when a secondary user is within the transmission range of an active primary user, the probability that it misdetects and thus regards a channel is available equals to the rate of false negative.
...
%DIF > When the clustering process is completed, we will examine the performances on robustness and cluster size control with the group truth spectrum availability.
%DIF > C3
}\DIFaddend \end{itemize}


\DIFdelbegin \DIFdel{Simulation }\DIFdelend \DIFaddbegin \DIFadd{The simulation }\DIFaddend consists of two parts, first we investigate the performance of centralized scheme and the distributed schemes in a small network, as there is no polynomial time solution available to solve the centralized problem.
...
The \DIFaddbegin \DIFadd{other parameters \ie the number of CR and PU, and their transmission ranges are given in the beginning of the respective simulation sections.
%DIF > C1-10
The }\DIFaddend constant $t$ which is used to control cluster size for ROSS (discussed in Section~\ref{ross_p2_cluster_pruning}) is 1.3.
...
\DIFdelbegin \DIFdel{There }\DIFdelend \DIFaddbegin \DIFadd{In this part of simulation, there }\DIFaddend are 10 primary users and 20 CR users dropped randomly (with uniform distribution) \DIFdelbegin \DIFdel{within }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend a square area \DIFdelbegin \DIFdel{of size $A^{2}$, where we set }\DIFdelend \DIFaddbegin \DIFadd{where side length is $A$.
$A$ is a positive value and }\DIFaddend the transmission ranges of \DIFaddbegin \DIFadd{both }\DIFaddend primary and CR users \DIFaddbegin \DIFadd{are set }\DIFaddend to $A/3$.
\DIFaddbegin \DIFadd{By doing this, we try to abstract from the influence of any given physical layer technology, and A can be given a concrete value in practice when the physical layer technology is decided.
%DIF > C1-10, C1-9
}\DIFaddend %There are 10 available channels. 
...
{%DIFAUXCMD
\DIFdelFL{Comparison between the distributed and centralized clustering schemes ($N$ = 20)}}
%DIFAUXCMD
\DIFdelendFL %DIF > \caption{Comparison between the distributed and centralized clustering schemes ($N$ = 20)}
...
\DIFaddbegin \DIFadd{If we use the number of secondary users residing in a cluster as the numerical representation of cluster robustness, then the variants of ROSS is from 5\%,  30\% and 230\% more robust than SOC with the number of newly added PR is 10, 40 and 80 respectively.
}\DIFaddend The above observation shows that the average number of CCs of non-singleton clusters doesn't necessarily illustrate the robustness  of cluster, \ie SOC obtains the most CCs for the clusters which are meanwhile the most vulnerable.
...
\begin{figure}[ht!]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.7\linewidth]{ccc_large_scale_color.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.7\linewidth]{ccc_large_scale_color_082017_newdata_no_texture.pdf}
  \DIFaddendFL \caption{Average number of CCs of non-singleton clusters}
  \label{ccc_large_scale}
\end{figure}

The average number of CCs of the non-singleton clusters is shown in Figure~\ref{ccc_large_scale}.
\DIFdelbegin \DIFdel{SOC achieves the most CCs per non-singleton cluster, but the lead over the variants of ROSS decreases significantly when $N$ increases.
%DIF < \ie when $N=300$, the number of CCs achieved by ROSS variants (except for ROSS-$\delta$-DFA) is almost the same as that resulted from SOC.
}\DIFdelend \DIFaddbegin \DIFadd{Here we don't see the obvious difference between different schemes on the number of CCs.
%DIF > SOC achieves the most CCs per non-singleton cluster, but the lead over the variants of ROSS decreases significantly when $N$ increases.
}\DIFaddend %This means SOC out performs in terms of the average number of CCs per non-singleton cluster when network is sparse.
...
\subsubsection{Robustness of the \DIFdelbegin \DIFdel{formed clusters}\DIFdelend \DIFaddbegin \DIFadd{Formed Clusters}\DIFaddend }
\DIFaddbegin \DIFadd{Here we see how vulnerable the clusters are when they are exposed to the increasing influence of the PUs.
}\DIFaddend We increase the primary users' activity by importing 20 batches of PUs sequentially in the CRN, where each batch includes 10 PUs. 
%
Figure~\ref{singleton_clusters_100} and \ref{singleton_clusters_200} show that when $N=100$ and 200, compared with the variants of ROSS, more unclustered CR nodes are \DIFdelbegin \DIFdel{generated by SOC }\DIFdelend \DIFaddbegin \DIFadd{caused by SOC with the same intensity of PUs' activities}\DIFaddend .
...
\begin{figure}[!h]
  \centering
   \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\linewidth]{nClusters_largeNetwork.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.7\linewidth]{nClusters_largeNetwork_no_texture.pdf}
  \DIFaddendFL \caption{The number of formed clusters.}
...
\DIFaddbegin \subsubsection{\DIFadd{The Performance with False Negative in Spectrum Sensing}}


\DIFadd{Figure~\ref{false_negative_ccc} shows the average number of CCs decrease slightly when the false negative rate increases.
The size distribution of the ROSS-DGA, ROSS-$\delta$-DGA and SOC is shown in Figure~\ref{false_negative_CDF}.
  ...
  \caption{\DIFaddFL{The number of CCs per non-singleton cluster with the presence of spectrum sensing false negative}}
  %DIF > , there are $x=6$ when $N=100$, $x=12$ when $N=200$, $x=21$ when $N=300$, which is around $2/3$ of the number of average neighbours.
  \label{false_negative_ccc}
\end{figure}
%DIF > The influence of the false negative on the sustainability of the clusters can be seen from Figure~\ref{false_negative_CDF}.
\DIFadd{For all the schemes, when false negative increases, the number of singleton clusters increases accordingly.
%DIF > As to the ROSS variants, the changes of the CDF curves are resulted from the increasing singleton clusters.
...
We can conclude that through negotiation within the neighborhoods, ROSS variants successfully rules out the channel from false negative sensing.
  ...
  \caption{\DIFaddFL{100 CRs with false negative in spectrum sensing, 30 PUs in network}}
  \label{false_negative_CDF}
\end{figure}

\DIFaddend \subsection{Insights Obtained from the Simulation}
The simulation \DIFdelbegin \DIFdel{made with the }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend large CRN network confirms \DIFdelbegin \DIFdel{that made with the }\DIFdelend \DIFaddbegin \DIFadd{the conclusion drawn from the }\DIFaddend small CRN network\DIFdelbegin \DIFdel{, which is that the }\DIFdelend \DIFaddbegin \DIFadd{.
First, different from the assumption adopted in all the previous works, the }\DIFaddend average number of CCs \DIFdelbegin \DIFdel{along doesn't }\DIFdelend \DIFaddbegin \DIFadd{per cluster can }\DIFaddend tell the robustness \DIFdelbegin \DIFdel{of the clusters, because the cluster size and the constitution of the cluster also affect the robustness.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{against the increasing influence of the primary users.
Second, the }\DIFaddend centralized clustering scheme is able to form the clusters which satisfy the requirement on cluster size strictly, and the clusters are robust against \DIFdelbegin \DIFdel{the PUs' }\DIFdelend \DIFaddbegin \DIFadd{PUs' increasing }\DIFaddend activity, besides, it involves the smallest control overhead in the process of clustering.
\DIFdelbegin \DIFdel{As }\DIFdelend \DIFaddbegin \DIFadd{Third, as }\DIFaddend distributed schemes, the variants of ROSS outperform SOC considerably \DIFdelbegin \DIFdel{on three metrics.
First, the }\DIFdelend \DIFaddbegin \DIFadd{in four aspects.
}\begin{itemize}
\item \DIFadd{The }\DIFaddend variants of ROSS generate less unclustered nodes than SOC for a given CRN, and the resulted clusters are more robust than SOC when PUs become more active.
\DIFdelbegin \DIFdel{Second, the }\DIFdelend \DIFaddbegin \item \DIFadd{The }\DIFaddend signaling overhead involved in ROSS is about half of that needed for SOC, and the signaling messages are much shorter that the latter.
\DIFdelbegin \DIFdel{Third, the }\DIFdelend \DIFaddbegin \item \DIFadd{The }\DIFaddend sizes of the clusters generated by ROSS demonstrate smaller discrepancy than that of SOC.
\DIFaddbegin \item \DIFadd{The variants of ROSS are more resilient against the imperfect spectrum sensing.
}\end{itemize}

\DIFaddend Moreover, the ROSS variants with size control features achieve similar performance to the centralized scheme in terms of cluster size, and the cluster robustness is similar when applying the variants of ROSS and the centralized scheme respectively.
%
\DIFdelbegin \DIFdel{As to }\DIFdelend \DIFaddbegin \DIFadd{Among }\DIFaddend the variants of ROSS, the greedy mechanism in ROSS-DGA helps to improve the performance on cluster size and cluster robustness at the cost of increased signaling overhead.
...
\section{Conclusion}
\label{conclusion}
In this paper we investigate the robust clustering problem in CRN \DIFdelbegin \DIFdel{extensively }\DIFdelend \DIFaddbegin \DIFadd{thoroughly }\DIFaddend and propose both centralized and distributed clustering solutions.
...
\DIFaddbegin \DIFadd{Besides, spectrum sensing errors affect the performance of the variants of ROSS much less than SOC.
}\DIFaddend 
...
\DontPrintSemicolon
\SetAlgoLined
\DIFdelbegin %DIFDELCMD < \KwIn{$d_j, g_j, j\in \text{Nb}(i)\setminus \Lambda$, $\Lambda$ means cluster heads. Empty sets $\tau_1,\tau_2$}
%DIFDELCMD < \KwResult{Returning 1 means $i$ is cluster head, then $d_j$ is set to 0, $j\in \text{Nb}(i)\setminus \Lambda$. returning 0 means $i$ is not cluster head.}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \KwIn{$d_j, g_j, j\in \text{Nb}(i)\setminus \Lambda$, $\Lambda$ denotes the set of cluster heads among $\text{Nb}(i)$. Empty sets $\tau_1,\tau_2$}
...
\section{Proof of \DIFdelbegin \DIFdel{Theorem}\DIFdelend \DIFaddbegin \DIFadd{Lemma}\DIFaddend ~\ref{clustering:theorem}}
\begin{proof}
\DIFdelbegin %DIFDELCMD < \label{proof_clustering:theorem}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{proof_clustering:lemma1}
\DIFaddend %Note that the formed cluster can be a singleton cluster, \ie cluster size is 1.
...
Then there \DIFdelbegin \DIFdel{is at least one }\DIFdelend \DIFaddbegin \DIFadd{exists a }\DIFaddend node $\beta\in \text{Nb}(\alpha)$ such that $d_{\alpha} > d_{\beta}$ (otherwise $\alpha$ becomes cluster head). 
...
If $\beta$ is not a cluster head, then $\beta$ is not in any cluster, we can repeat the previous analysis made on node $\alpha$, and deduce that node $\beta$ has \DIFdelbegin \DIFdel{at least one }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend neighboring node $\gamma$ with $d_{\gamma} < d_{\beta}$.
...
But this list will not continue \DIFdelbegin \DIFdel{to grow}\DIFdelend \DIFaddbegin \DIFadd{growing}\DIFaddend , because the minimum individual connectivity degree is zero, and the length of this list is upper bounded by the total number of nodes in the CRN.
...
\DIFdelbegin \DIFdel{If we see a secondary user }\textit{\DIFdel{becoming a cluster head}}%DIFAUXCMD
\DIFdel{, or }\textit{\DIFdel{becoming a cluster member}} %DIFAUXCMD
\DIFdel{as one step, as the length of the list of secondary users is not larger than $N$, there are $N$ steps for this scenario to form the initial clusters.
}\DIFdelend %DIF > If we see a secondary user \textit{becoming a cluster head}, or \textit{becoming a cluster member} as one step, as the length of the list of secondary users is not larger than $N$, there are $N$ steps for this scenario to form the initial clusters.
