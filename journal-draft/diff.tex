...
%DIF ADD ettdoc.tex        Sun Sep 24 21:45:19 2017
...
}\DIFdelend \DIFaddbegin \DIFadd{robust clustering in CRN is analyzed and both centralized and distributed schemes are proposed.
This paper is based on our previous work ROSS (Robust Spectrum Sharing)~\mbox{%DIFAUXCMD
\cite{Li11_ROSS}}%DIFAUXCMD
, but fundamental extensions are made in several aspects.
This paper gives a comprehensive analysis of robust clustering problem, analyze and investigate the influence of cluster size and the number of CCs on cluster robustness.
We claim that the cluster robustness is no longer indicated merely by the average number of CCs of clusters, but by the ability of the clusters to uphold when the primary users' activity becomes more active.
The involment of cluster size enables us to solve the size devergence problem in clusters, which appears in~\mbox{%DIFAUXCMD
\cite{Li11_ROSS} }%DIFAUXCMD
and~\mbox{%DIFAUXCMD
\cite{LIU_TMC11_2}}%DIFAUXCMD
.
Based on ROSS, our proposed distributed schemes are still suitable for the scenario where fast deployment is desired.
Throughout this paper, we refer to the clustering schemes on the basis of ROSS as the }\textit{\DIFadd{variants of ROSS}}\DIFadd{.
%DIF > 
%DIF > 
...
\DIFaddbegin \section{\DIFadd{Related Works and the Relation with This Paper}}
\label{related_work}
...
As there is no restriction on the nodes to access the spectrum in wireless ad-hoc, mesh networks andsensor networks, the objective of clustering is to decrease the transmission power consumption~\mbox{%DIFAUXCMD
\cite{Kawadia03}}%DIFAUXCMD
, to improve the routing performance~\mbox{%DIFAUXCMD
\cite{clustering_mesh_globecom2010}}%DIFAUXCMD
, or to imrove the network lifetime and coverage~\mbox{%DIFAUXCMD
...
As introduced in Section~\ref{intro}, \DIFdelbegin \DIFdel{in order to be robust against primary users' activity, the }\DIFdelend \DIFaddbegin \DIFadd{robustness of the clusters is their ability to uphold with the influence of the active primary users, and it is represented by the number of secondary users which are not included in any cluster.
To achieve better robustness, the }\DIFaddend formed clusters should have more CCs.
...
ROSS consists of two cascaded phases: \textit{cluster formation} and \textit{membership clarification} \DIFdelbegin \DIFdel{.
...
\DIFaddend In the first phase, clusters are formed quickly and every CR user becomes either a cluster head or a cluster member, besides, cluster size control is implemented in this phase.
...
\DIFaddbegin \DIFadd{Connectivity vector is calculated by every secondary user after channel availability is obtained, and is then broadcasted.
...
\DIFaddbegin \DIFadd{Actually during the whole phase I, whenever a CR node is decided to be a cluster head thus accordingly forms a cluster, or its cluster's composition is changed, the cluster head will broadcast the new information about its cluster, which includes the sets of available channels on all its cluster members}\DIFaddend .
...
We have the following \DIFdelbegin \DIFdel{theorem }\DIFdelend \DIFaddbegin \DIFadd{lemma }\DIFaddend to show that every secondary user will eventually be either integrated into a certain cluster or becomes a cluster head.
...
\DIFaddend Given a CRN \DIFdelbegin \DIFdel{, it takes at most $N$ steps that }\DIFdelend \DIFaddbegin \DIFadd{where any secondary user is able to communicate with any other secondary user through the other nodes, then after the phase of cluster head selection and initial cluster formation, }\DIFaddend every secondary user either becomes cluster head, or gets included into at least one cluster.
...
\DIFaddend The Proof is \DIFaddbegin \DIFadd{given }\DIFaddend in Appendix~\DIFdelbegin \DIFdel{\ref{proof_clustering:theorem}.
}\DIFdelend \DIFaddbegin \DIFadd{\ref{proof_clustering:lemma1}.
...
\DIFdelbegin \DIFdel{The procedure of the proof also illustrates the maximal time needed to conduct Algorithm~\ref{clustering:theorem}.
...
}\DIFdelend %DIF > Assuming there is a token in the CRN, one node is allowed to execute Algorithm~\ref{alg0} only when it holds the token, and it then transfers the token to the neighboring node on the chain.
...
\DIFaddend %
\DIFdelbegin \DIFdel{If one step can be finished within a certain time span $T$,
then the total time needed for }\DIFdelend %DIF > This judgement is conducted periodically, and phase I ends after every node ascertains it is cluster head or not.
\DIFaddbegin 

\begin{lemma}
\label{clustering:lemma3}
\DIFadd{In the process of cluster head selection and initial cluster formation, }\DIFaddend the \DIFdelbegin \DIFdel{network to conduct Algorithm~\ref{clustering:theorem} is $N*T$.
}\DIFdelend \DIFaddbegin \DIFadd{maximum number of times that a secondary node becomes cluster head is $N$.
}\end{lemma}
...
\DIFaddend As Algorithm~\ref{alg0} can be executed concurrently by different secondary users, the needed time can be considerably reduced.
%DIF < According to Theorem~\ref{clustering:theorem}, Phase I completes within a reasonable amount of time.
%DIF < 
%DIF < This judgement is conducted periodically, and phase I ends after every node ascertains it is cluster head or not.
If we apply Algorithm~\ref{alg0} to the example shown in Figure~\ref{fig1}, \DIFdelbegin \DIFdel{then }\DIFdelend the outcome is shown in Figure~\ref{fig2}.
