...
\item For all clusters with size within the range $\big[\delta_1, \delta_2\big]$, with $\delta_1, \delta_2\in \mathbb{Z}^+$ and $\delta_1 \leq \delta_2$, the number of CCs per cluster is $f(C) = K(C)$, i.e. it is given as the number of jointly available CCs for all nodes of that cluster. The range can also be expressed as $[\delta-u, \delta+u]$, where $\delta$ is referred to as \textbf{desired cluster size}, $u$ is a constant and $\delta-u > 0$.
...
Define the set $\mathcal{S}$ as the powerset of $\mathcal{S}$, i.e. it contains all possible clustering possibilities that can be formed of $\mathcal{N}$, with $\mathcal{S}=\{C_1, C_2,\ldots,C_i, \ldots, C_{|\mathcal{S}|}\}$ where subscript $i$ refers to the $i$-th cluster in the powers $\mathcal{S}$.
...
% XXX I have no clue what you trying to express with these three lines.
